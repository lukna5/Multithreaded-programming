...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private fun acquire(index: Int, op: Op): AcquiredAccount? {
        ...
        while (true) {
            ...
            if (!account.invokeOperation()) {
                val acquiredAcc = AcquiredAccount(account.amount, op)
                if (accounts[index].compareAndSet(account, acquiredAcc)) {
                    return acquiredAcc
                }
            }
        }
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
             ...
             * Basically, implementation of this method must perform the logic of the following code "atomically":
             */
            val fromIndex1 = min(fromIndex, toIndex)
            val toIndex1 = max(fromIndex, toIndex)
            val minAc = acquire(fromIndex1, this)
            val maxAc = acquire(toIndex1, this)
            if (minAc != null) {
                if (maxAc != null) {
                    var from = minAc
                    var to = maxAc
                    if (fromIndex1 != fromIndex){
                        from = maxAc
                        to = minAc
                    }
                    when {
            ...
            completed = true
            release(toIndex1, this)
            release(fromIndex1, this)
        }
    }
}